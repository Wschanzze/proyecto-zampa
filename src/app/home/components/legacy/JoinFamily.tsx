...
const JoinFamily = () => {
  const contacts = [
    {
      icon: (
        <svg className="w-5 h-5 text-wheat-light" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <section className="relative w-full overflow-hidden py-24 px-6 lg:px-12 flex flex-col items-center justify-center gap-16 md:gap-20">
      ...
      <div className="relative z-20 max-w-4xl mx-auto text-center flex flex-col items-center gap-6">
        <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-wheat-light">UNITE A LA FAMILIA ZAMPA</span>
      ...
      <div className="relative z-20 max-w-7xl mx-auto w-full border-t border-white/10 pt-16">
        <div className="text-center mb-10">
          <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-wheat-light opacity-80 mb-2 block">ATENCIÓN Y CONSULTAS</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
          {contacts.map((item, idx) => (
            <div key={idx} className="flex gap-4 p-6 rounded-xl bg-white/[0.04] backdrop-blur-[2px] hover:bg-white/[0.08] border border-white/10 hover:border-white/20 shadow-lg hover:shadow-xl transition-all duration-300 items-start text-left">
              <div className="flex-shrink-0 w-10 h-10 rounded-full bg-wheat-light/10 flex items-center justify-center text-wheat-light">
                {item.icon}
              </div>
              <div className="flex flex-col min-h-[60px] justify-center">
                <h4 className="text-xs font-bold tracking-wider uppercase text-white/50 mb-1.5">{item.title}</h4>
                {item.href ? (
                  <a 
                    href={item.href}
                    target={item.href.startsWith('http') ? "_blank" : undefined}
                    rel={item.href.startsWith('http') ? "noopener noreferrer" : undefined}
                    className="text-sm font-light text-white hover:text-wheat-light transition-colors whitespace-pre-line leading-relaxed"
                  >
                    {item.desc}
                  </a>
                ) : (
                  <p className="text-sm font-light text-white/95 whitespace-pre-line leading-relaxed">{item.desc}</p>
                )}
              </div>
            </div>
          ))}
